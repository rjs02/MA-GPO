...
#SBATCH --job-name=train_gpm_hh_4b_4gpu
#SBATCH --mail-type=BEGIN,END

# Training script for GPM on Anthropic/hh-rlhf dataset
# Using 4x RTX 4090 GPUs with DeepSpeed ZeRO-3 for Qwen3-4B
# Effective batch size: 128 (tight on memory but should work)
...
MICRO_BATCH_SIZE=2      # Per-GPU batch size (keep at 2 for memory safety)
ACCUMULATED_GRADIENT=16 # Gradient accumulation steps (higher to reach batch size 128)
# Effective batch size = MICRO_BATCH_SIZE * ACCUMULATED_GRADIENT * NUM_GPUS = 2 * 16 * 4 = 128

DATE=$(date +%Y%m%d_%H%M%S)

export EXP_NAME="qwen3-4b-gpm-dim${VALUE_HEAD_DIM}-hh-rlhf-4gpu-${DATE}"
...
echo ""
echo "Memory note: Using ZeRO-3, expect ~21-22GB per GPU (tight but should work)"
