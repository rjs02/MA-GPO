...
ACCUMULATED_GRADIENT=16 # Gradient accumulation steps
# Effective batch size = MICRO_BATCH_SIZE * ACCUMULATED_GRADIENT * NUM_GPUS = 2 * 16 * 4 = 128

DATE=$(date +%Y%m%d_%H%M%S)

export EXP_NAME="qwen3-4b-gpm-dim${VALUE_HEAD_DIM}-hh-rlhf-4gpu-offload-${DATE}"
...
echo ""
echo "Memory optimization: Using ZeRO-3 with optimizer + parameter offloading to CPU"
echo "Expected GPU memory: ~13-15GB per GPU (very safe)"
echo "Note: Training will be 15-20% slower due to CPU offloading"
